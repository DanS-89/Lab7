...
public class ArraysAndStreams2 {
   public static void main(String[] args) {
      ...
      List<String> vowelString = Arrays.stream(strings)
              .filter(s -> s.matches("(?i)^[aeiouAEIOU].*"))
              .collect(Collectors.toList());
               System.out.println("strings that start with a vowel: " + vowelString);

      String concatString = Arrays.stream(strings)
      ...
      long count = Arrays.stream(strings)
              .filter(s -> s.length() > 5)
              .count();
               System.out.println("Number of strings with more than 5 elements: " + count);
   }
} 
